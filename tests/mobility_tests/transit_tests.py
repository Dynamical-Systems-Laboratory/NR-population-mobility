...
def creating_public_transit_objects(transit):
    ...
    transit.create_public_transit(cp1[0], cp1[1], cp1[2], cp1[3])
    transit.create_public_transit(cp2[0], cp2[1], cp2[2], cp2[3])
    transit.create_public_transit(cp3[0], cp3[1], cp3[2], cp3[3])
    ...
    if (public_transit[0].work_destination != cp1[3]) or  (public_transit[1].work_destination != 0):
    ...
    with open(fname, 'r') as fin:
        for line in fin:
            ...
            if not math.isclose(float(line[2]), public_transit[n_count-1].travel_time):
                ...
                return False
            if not math.isclose(float(line[3]), public_transit[n_count-1].work_destination):
