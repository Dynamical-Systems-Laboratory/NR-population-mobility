...
class Agents(object):
	...
	def distribute_transit_and_workplaces(self, households, workplaces, transit, max_working_age, n_employed):
	...
	def __repr__(self):
		...
		for agent in self.agents:
			temp.append((' ').join([str(int(agent['student'])), str(int(agent['works'])), 
									...
									str(agent['carpoolID']), str(agent['publicID'])])) 
